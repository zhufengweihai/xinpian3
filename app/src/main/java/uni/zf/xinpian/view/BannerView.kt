...
import uni.zf.xinpian.recommend.BannerImageAdapter
...
class BannerView(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    fun setVideoList(videoList: List<Video>, corner: Boolean) {
        ...
        updateNameView(videoList.first().name)
    }

    private fun setupViewPager(videoList: List<Video>) {
        binding.viewPager.adapter = BannerImageAdapter(videoList)
    ...
    private fun displayInLoop(videoList: List<Video>) {
    ...
    private fun updateCurrentView(nextItem: Int, videoList: List<Video>) {
        binding.viewPager.setCurrentItem(nextItem, true)
        binding.indicatorGroup.check(nextItem)
        updateNameView(videoList[nextItem].name)
    }
